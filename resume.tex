...
Hint:
\begin{align*}
\frac{\partial \log p(x_i)}{\partial h_k} = \frac{1}{p(x_i)}\frac{\partial p(x_i|h_k)P(h_k)}{\partial h_k} \\
= \frac{p(x_i|h_k)P(h_k)}{p(x_i)}\frac{\partial \log p(x_i|h_k)}{\partial h_k} \\
= P(h_k|x_i) \frac{\partial \log p(x_i|h_k)}{\partial h_k}
\end{align*}
...
\line(1,0){140} \\
\tiny
PCML2012 Cheat sheet kindly written by Christopher Chiche, Alex Chap, Tobias Schlatter, Florian Savoy, Marc Zimmerman and Julien Perrochet. Copyleft EPFL 2012 
\end{multicols}
\end{document}
